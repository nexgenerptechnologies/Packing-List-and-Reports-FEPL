...
class PartnerSubAllocation(Document):
	def validate(self):
		self.validate_quota()
	...
	def validate_quota(self):
		...
		if total_allocated > flt(self.quota):
			frappe.throw(_("Total allocated quantity ({0}) exceeds your assigned quota ({1})").format(total_allocated, self.quota))
